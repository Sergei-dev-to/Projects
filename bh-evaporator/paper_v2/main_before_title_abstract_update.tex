...
\title{A Non-Gravitational Sector-Rate Model for Black-Hole-Like Evaporation}
...
\begin{abstract}
We construct a finite non-gravitational comparison model for several features
usually bundled with black-hole evaporation.  The primary object is a
Hamiltonian-derived sector-rate model.  The core Hilbert space is a direct sum
of finite sectors with an area-like state count, the sector mass obeys a
Schwarzschild-like relation, and outgoing radiation is produced by couplings
from one sector to a smaller sector plus a radiation mode.  In the secular
limit the rates are determined by transition matrix elements, radiation phase
space, and the density of final core states.  With energy-resolved sectors, an
exponential local density of states gives a hard spectrum close to the thermal
form in the variable $\beta\omega$; adding idealized within-sector mixing gives
accelerating emission for the square-root mass law, while the corresponding
linear mass-law control remains approximately non-accelerating.  A separate
finite repeated-interaction realization tracks purification and information
flow.  On one fixed eight-emission schedule it shows a small Page-like turnover
of the full radiation von Neumann entropy together with the shrinking
core-support bound.  The sector dimensions, mass law, local density of states,
and mixing assumption are inputs.  The result is an explicit diagnostic model:
it shows which parts of the evaporation package can be assembled from finite
quantum statistical mechanics, and which parts still require a microscopic
origin.
\end{abstract}
...
To regenerate the central CSV outputs from the project root:
\begin{verbatim}
