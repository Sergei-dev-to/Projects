#!/usr/bin/env python3
"""Generate a summary figure for the reduced-density Hamiltonian channel."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--convex",
        type=pathlib.Path,
        default=DATADIR / "hamiltonian_shell_density_curv3_ch8_g05_s48.npz",
    )
    parser.add_argument(
        "--control",
        type=pathlib.Path,
        default=DATADIR / "hamiltonian_shell_density_linear_ch8_g05_s48.npz",
    )
    parser.add_argument(
        "--output",
        type=pathlib.Path,
        default=PROJECT / "hamiltonian_density_summary.pdf",
    )
    ...
    ax = axes[0, 0]
    ax.plot(convex["energies"], convex["temperature"], marker="o", label="convex")
    ax.plot(control["energies"], control["temperature"], marker="s", label="linear")
    ax.invert_xaxis()
