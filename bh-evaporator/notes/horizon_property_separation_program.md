...
| Near-extremal / JT throat | yes | yes, with large `S0` plus active entropy | bath-coupled island setups common | diagnostic: zero-line BPS silent; emitted line may be `S0`-sized, active-entropy-sized, or quantum-enhanced | Schwarzian/SYK-like expectation | low-temperature throat | Dangerous case: distinguishes no-line degeneracy, semiclassical area cross-section, active-entropy throttling, and strong-Schwarzian enhancement. |
...
Status: lemma plus theorem-class bridge note.  The visible-algebra
obstruction is in `operator_overlap_latency_lemma.md`; the bounded
source-local generator-defect bridge is in
...
`operator_overlap_latency_lemma.md`; a bounded-budget source-local
bridge theorem class is drafted in
`q2_operator_overlap_bridge_theorem.md`.
...
The bridge theorem should be stated with the integrated distance of the
source-local record generators from a diary-blind algebra.  In bounded
budget record channels, small integrated defect makes the record
channel diamond-close to diary-independent; HP-fast recovery therefore
requires order-one generator access, nonlocal/constraint-dressed
emission access, or supplied side information.  The older instantaneous
`G_D(t)` projection profile remains a useful diagnostic, but replacing
the generator-defect theorem by `G_D(t)` requires a separate norm-duality
lemma.  This is the calibrated way to survive the Lie-algebra-generation
warning.  The strict memory-burden prototype sits at the exact-zero
defect endpoint for fixed-sector coherences.
...
2. Run or sketch the leg-A support calculation for the tag-multiplexing
   ...
   near-Poissonian `g2` inside resolved burden-tag bins, while broadband
   integration over detuned bins can mimic chaotic bunching.

3. Turn Theorem B into a precise two-line lemma about collective
...
4. Polish Theorem C/Q2 for paper form: fix the source-local
   bounded-budget record class, state the blind-algebra generator-defect
   bound with constants, and add the optional norm-duality lemma only if
   the older `G_D(t)` projection profile is used.  The memory-burden
   prototype remains the strict zero-defect countermodel.

5. Write the Rindler null lemma as a short appendix-level note:
   modular thermality yes; finite recoverable register no.  This should
   be a control, not a new paper.

6. For BTZ, identify the CFT image of `N_eff`: likely an operator
   participation or OPE-channel participation measure in a thermal
   band, not a bulk source-cell count.

7. Add a passive/active channel scope note to the saturation paper:
   natural radiation and specified bath protocols are the target;
   mining is a different exterior coupling.

8. Add references/positioning for edge modes and crossed products on
