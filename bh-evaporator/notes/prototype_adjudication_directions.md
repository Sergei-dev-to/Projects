...
Status: the first bridge in `q2_operator_overlap_bridge_theorem.md` has been
superseded by the exact long-time result in
`q2_composable_diary_access_theorem.md`. The obstruction can be expressed by
...
The invariant result is stated at comparison-process level. If `eta_j` is the
actual-versus-blind step defect on every hybrid-reachable state, then

```text
||N_K-C_K||_(code,diamond) <= sum_j eta_j.
```

For collision Hamiltonians, `eta_j <= 2 |g_j| eps_j`, with
`eps_j=||H_j-H_j^blind||`; Duhamel plus channel contractivity gives no
exponential in the total blind evolution budget. Therefore recovery requires
order-one cumulative process defect, nonlocal/constraint-dressed access, or
supplied side information even for persistent pumps and changing shells. The
older single-time `G_D(t)` profile remains only a routing diagnostic unless a
norm-duality lemma connects it to the process defect.
...
```text
1. DONE: stable long-time diary-visibility bound for arbitrary memoryful
   sequential processes; the defect accumulates linearly, not exponentially.
2. DONE at kinematic level: blind/weak/mixing emitters on shrinking
   S(E) ~ E^2 shells, plus an exact locally identical five-mode code emitter.
3. ACTIVE: generate the mixing temporal code with the finite pump/autonomous
   shell dynamics, or verify it in a named microscopic gravity model. The
   abstract integrated collision is now done; the conditional sufficient side
   is the existing ETH/design decoupling theorem. Further progress requires a
   microscopic target and its physical emission amplitudes.
4. Join that operational result to the radiation-algebra/factorization seam.
5. Preserve passive HIGH/LOW, starvation-flash, and two-drain results as
   phenomenology and possible tabletop diagnostics.
6. Keep the M-series strict prototype as an adjudicated excluded branch, not
   as the universal black-hole channel.
7. Use the novelty audit before choosing the next flagship paper.
```
