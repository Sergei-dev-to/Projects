...
The shrinking-shell control, composable Q2 theorem, and integrated
microcanonical coded collision have now closed the abstract pieces: exact
`S(E)~E^2` shell bookkeeping, blind/weak/mixing routing, stable long-time
necessary access, density-of-states thermal event odds, and correlation-only
recovery from locally identical records. They are not yet generated by a
simple autonomous Hamiltonian.

1. Find a single autonomous Hamiltonian that generates the integrated
   microcanonical collision, greybody mixing,
   and the changing temperature without step-dependent control.
2. Generate a diary-mixing temporal code with locally Hawking-like modes from
   that Hamiltonian, or verify an ETH/design condition for a named microscopic
   emitter.
3. Apply the standard decoupling sufficiency theorem uniformly along the
   resulting physical shell trajectory.

These are now one integrated positive-construction problem rather than four
independent formal gaps.
