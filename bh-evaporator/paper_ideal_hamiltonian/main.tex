...
\title{Schwarzschild Evaporation Phenomenology from a Non-Gravitational Hamiltonian}
...
\begin{abstract}
We give a non-gravitational Hamiltonian construction that reproduces the
exterior thermodynamic and fine-grained entropy phenomenology of Schwarzschild
evaporation from four inputs: a subsystem with Schwarzschild density of states,
an outgoing radiation continuum, weak energy-conserving emission from
area-many boundary-accessible operators, and microcanonical mixing.  The
weak-coupling calculation gives a locally thermal radiation spectrum, the
leading finite-energy correction, negative heat capacity, and the
Schwarzschild number-flux, power, and lifetime scalings.  The area dependence
of the rate follows from operator counting in the boundary-accessible emission
algebra.  Global purity, shrinking remaining Hilbert-space dimension, and
standard decoupling estimates give the Page curve and post-Page early/late
radiation correlations.  At second R\'enyi order the purity contains two
competing contractions, yielding the island min formula with the gravitational
area term replaced by the microcanonical entropy of the remaining subsystem.
Approximate-design and tensor-product-expander constructions give explicit
finite-regulator realizations of the mixing assumption.  Replacing this
abstract mixer by a
simple deterministic sparse Hamiltonian is a separate dynamical problem: a
Cayley-expander candidate makes the boundary emission weights equal by
symmetry, while the remaining requirement is sectorwise cumulative
second-moment decoupling for the induced weak-emission shrinking channel.  The
construction addresses exterior entropy flow; it does not include interior
geometry, horizon smoothness, or entanglement-wedge reconstruction.
\end{abstract}
...
Semiclassical black-hole evaporation combines thermodynamic regularity with a
subtle fine-grained information problem.  For an uncharged nonrotating black
hole,
\begin{equation}
...
non-gravitational model: the two candidate entropies are the emitted
thermodynamic entropy and the microcanonical state count of the remaining
object.
...
\cite{BasuWenZhou2025}.  Here the same entropy minimum is tied to an
energy-resolved evaporation Hamiltonian and to the Schwarzschild rate law.
...
The paper separates Schwarzschild evaporation phenomenology from the
Hamiltonian assumptions used to reproduce it.  The exterior features recovered
...
$M$.  The strengthened version represents this state count by
boundary-accessible degrees of freedom.  It is weakly coupled to an outgoing
radiation continuum by an energy-conserving interaction whose local emission
operators act on those boundary-accessible degrees.  The inclusive
low-frequency emission strength is then proportional to the number of active
boundary emitters, hence to the area.  This reproduces the area scaling of the
standard low-frequency absorption cross-section by an operator-counting
mechanism.  Mixing inside microcanonical shells is assumed to be
cumulative over the evaporation history.  The fine-grained Page-curve and
information-transfer statements require decoupling of the reference system from
the subsystem that should be ignorant of the code.  In the design-based
version used below, the regulated in-shell mixing blocks are chosen from
approximate-design or tensor-product-expander constructions, so this
decoupling condition follows from standard results
...
These assumptions give the thermodynamic features above through standard
weak-coupling and microcanonical arguments.  The
density-of-states ratio gives the local thermal factor and the leading
finite-energy correction.  The area-sized channel count gives the
Schwarzschild quanta rate, power, and lifetime scalings.
The falling Page branch follows from global purity after the remaining
black-hole Hilbert space becomes smaller than the radiation support.  The
rising branch and post-Page early/late correlations follow from Page's theorem
and decoupling applied to the composite emission isometry.

The boundary-accessible density of states is an input to the Hamiltonian.  A
derivation of that input from simpler non-gravitational microscopic degrees of
freedom is a separate problem.  Given that input, local weak coupling to the
boundary-accessible degrees gives the area emission strength, and the listed
exterior phenomenology follows from Hamiltonian evolution in the weak-coupling
regime together with design-based decoupling of the composite emission map
for the fine-grained information-theoretic statements.  The model has no
interior geometry and
therefore does not address
horizon smoothness, island reconstruction from the radiation, complementarity,
or firewall questions.
...
$N(E)$ define nearby evaporators with the same microcanonical thermality but a
different total power law.
...
entropy and local weak emission.
...
decouple the reference from the subsystem that should not contain the code
information.  This is the channel-scrambling input supplied by the shell mixer.
...
The comparison with existing models isolates the contribution of the present
construction: it combines Page's isometry logic with an energy-resolved
Hamiltonian whose boundary-accessible density of states and local weak
emission reproduce the Schwarzschild thermodynamic scalings.  The radiation
entropy also has the island-formula structure, and its second R\'enyi purity has
the corresponding two-term replica competition, with the gravitational area
term replaced by the microcanonical entropy of the remaining subsystem.
