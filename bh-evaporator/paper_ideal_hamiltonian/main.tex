...
to the radiation at the Page transition.
...
decoupling, with errors controlled by the golden-rule parameter;
...
mixers with the same state count fail.
...
has no interior geometry and does not address horizon smoothness,
complementarity, or firewall questions; the recovery statements of
Section~\ref{sec:recovery} concern code information, not interior geometry.
...
island-formula contraction, and derives the recovery and
complementary-reconstruction consequences.  Section 5 gives the
finite-dimensional
...
\end{tabular}
\end{center}
This table is a dictionary for the entropy and recovery statements only.  The
Hamiltonian model has no spacetime region bounded by a quantum extremal
surface and no gravitational path integral.  The last two rows are derived in
...
reconstruction, not a geometric one.  Second, the statements are conditional
...
and Appendix~\ref{app:cayley-scaling} adds
...
correlations, and complementary recovery of code information all follow from
...
above.  Recent work indicates how unusual such spectra are
\cite{AurellMajumdar2025}.
\item
...
entanglement forbids both \cite{AMPS2013}.  The model takes the trivial exit:
it has no interior, so nothing clashes.  Its recovery statements are the
algebraic content of island reconstruction, not a reconstruction of interior
geometry.  Proposition~\ref{prop:complementary} sharpens the question rather
than answering it: after the Page transition the code information is in the
radiation, so whatever the interior is in the gravitational description, it
must be assembled from radiation degrees of freedom --- the question
addressed by state-dependent interior constructions and the ER=EPR proposal
\cite{PapadodimasRaju2013,MaldacenaSusskind2013}.
\item
...
model has a single Hamiltonian and produces the same two contractions as
chaotic-channel statistics, which sharpens the question: why does geometry
implement this contraction?  Relatedly, small corrections to Hawking's
...
implement, and whether the interior sits behind a complexity barrier is a
part of the puzzle that existence statements do not touch
\cite{HarlowHayden2013}.
\item
...
Page value at the Haar rate.  The free family diverges
