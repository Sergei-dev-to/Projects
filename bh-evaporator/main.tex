...
\begin{abstract}
We propose and analyze a minimal non-gravitational quantum system that emulates key features of black-hole (BH) evaporation: (i) a negative heat-capacity window in which the emitter heats up as it loses energy, (ii) a near-thermal outgoing spectrum with a tunable surface-gravity analogue $\kappa(E)$, (iii) an evaporation law $P(E)\propto 1/E^2$ yielding an accelerating trajectory $E(t)=(E_0^3-3\beta t)^{1/3}$, and (iv) a Page-like turnover of the radiation entropy measurable via randomized measurements as a R̩nyi-2 lower bound. Our ``scaling dictionary'' (a ``Rosetta stone'') maps exponents controlling temperature, typical emission frequency, and in-band spectral slope to the evaporation exponent, reproducing the Schwarzschild $dM/dt\propto -1/M^2$ scaling. We formulate a precise ``lab paradox'' theorem for exact thermality + CP-divisible dynamics, show how our device departs at late times (microstate dependence and non-Markovian backflow), and outline a decoding task analogous to entanglement-wedge reconstruction. We emphasize pre-Planckian BH comparisons and avoid geometric claims.
\end{abstract}

\maketitle

\section{Introduction \& Motivation}
Black holes radiate at $T_H=\hbar\kappa/(2\pi k_B)$ and evaporate; the fine-grained entropy of Hawking radiation rises and then falls (Page curve) once replica-wormhole/QES effects are accounted for. In gravity, the fine-grained entropy is computed via quantum extremal surfaces (QES) that extremize the generalized entropy and can undergo a phase transition reproducing the Page curve in evaporating setups~\cite{EngelhardtWall2015,AEMM2019,Penington2019,AlmheiriRMP2021}. Throughout, ``Page curve'' refers to the von Neumann entropy in gravity (computed via QES); our experiments report a R̩nyi-2 lower bound that exhibits a Page-like turnover (see App.~E for bounds).
...
\caption{Ohmic passband $J(\omega)\propto \omega$ used to engineer the rate law.}
\end{figure}
...
\caption{Transition frequency mapping $\omega_\ast(E)=\gamma/E$ across the negative-$C_\mu$ window.}
\end{figure}
...
\caption{Normalized evaporation trajectories for BH mass and lab energy: $(1-3\tilde t)^{1/3}$.}
\end{figure}
...
\noindent\textbf{Theorem D.2 (R̩nyi-2 monotonicity in expectation under scrambling).} With exact $\sigma_k$, independent unitary 2-design scrambling on the core, and CP-divisible radiation dynamics, $\mathbb{E}[S^{(2)}_{\rm rad}]$ is nondecreasing (concentration bounds apply).\\
...
\caption{Toy microcanonical entropy $S(E)$ showing a convex intruder (negative $C_\mu$ window).}
\end{figure}

\begin{figure}[t]\centering
\IfFileExists{fig1b_T.pdf}{\includegraphics[width=\linewidth]{fig1b_T.pdf}}{\typeout{Missing figure fig1b_T.pdf}}
\caption{Microcanonical temperature $T_\mu(E)$ and an $1/E$ fit.}
\end{figure}

\section{Measurement protocols}
\paragraph*{What we certify (R̩nyi-2 vs.\ von Neumann).}
...
\caption{Synthetic R̩nyi-2 Page-like turnover; dashed line marks $t_{\rm Page}$.}
\end{figure}
...
\caption{Microstate distinguishability $D(t)$.}
\end{figure}
...
See Theorem D.1 (unital) and D.2 (R̩nyi-2 in expectation).
...
Core: fully connected XXZ with weak disorder, $N\in[12,16]$, edge $X=\frac{1}{\sqrt{N}}\sum_i \sigma_i^-$. We estimate $S(E)$, $T_\mu(E)$, $C_\mu(E)$, extract $\omega_\ast(E)$ from $A_X(\omega;E)$, form $P(E)$ with $J(\omega)\propto \omega$, and synthesize bins to show a R̩nyi-2 Page-like turnover. Code/data intended as ancillary files.
