...
\begin{abstract}
We introduce a geometry-free, unitary quantum ``evaporator'': a fast-scrambling core coupled to a single bright emission channel, featuring a finite microcanonical negative-heat-capacity window.
Within a calibrated band we establish the scaling dictionary $\omega_\ast(E)\!\sim\!1/E$ and $J(\omega)\!\sim\!\omega$ $\Rightarrow$ $P(E)\!\propto\!1/E^2$ and $E(t)\!=\!(E_0^3\!-3\beta t)^{1/3}$; an operational surface-gravity analogue follows from edge KMS/response, $\kappa(E)\!\propto\!1/E$.
Information-theoretically, expected R\'enyi-2 of radiation is monotone under CP-divisible (unital) dynamics; the observed Page-like turnover thus diagnoses late-time microstate dependence and CP-indivisible backflow.
Exact-diagonalization validates the ingredients (convex intruder, $\omega_\ast$ scaling); we calibrate $\beta$ from $P(E)$ and overlay the parameter-free $E(t)$ trajectory. We outline measurement/decoding protocols and feasible realizations (cQED, photonics, ultracold).
\end{abstract}
...
\\paragraph*{Contributions.}\n\\begin{itemize}[leftmargin=*]\n\\item \\textbf{Unitary evaporator architecture:} fast-scrambling core + single bright channel with a finite microcanonical negative-\\mu$ window; early near-KMS edge correlators despite global purity.\n\\item \\textbf{Scaling dictionary and evaporation law:} with $\\omega_\\ast(E) \\sim 1/E$ and in-band (\\omega) \\sim \\omega$, we obtain bh-evaporator/main.tex(E) \\propto 1/E^2$ and (t)=(E_0^3-3\\beta t)^{1/3}$; we define an operational $\\kappa(E)$ from edge KMS/response. Calibration of $\\beta$ from Fig.~\\ref{fig:PofE} yields a parameter-free overlay for Fig.~\\ref{fig:EtTraj} (Eq.~\\eqref{eq:PofE}).\n\\item \\textbf{Information-theoretic monotonicity vs. turnover:} we state that expected R\\'enyi-2 is nondecreasing under CP-divisible (unital) dynamics; the observed Page-like turnover arises from late-time microstate dependence and CP-indivisible backflow.\n\\item \\textbf{Measurement and decoding:} randomized measurements/shadows for ^{(2)}$; distinguishability (t)$ and a BLP-type witness; decoding via Petz/twirled-Petz/variational methods with sample complexity vs. detection efficiency $\\eta$.\n\\item \\textbf{Validation and robustness:} ED of a small-$ XXZ core (convex intruder, $\\omega_\\ast$ scaling); sensitivity to (\\omega)$, coupling, and $.\n\\item \\textbf{Experimental blueprint:} Purcell/two-pole filters to realize (\\omega) \\sim \\omega$ and isolate a single bright channel via impedance matching; viable platforms (cQED, photonics, ultracold) and controls/failure modes.\\end{itemize}\n\n
...
Under CP-divisible (Markovian) reduced dynamics with unitality in the relevant subspace, the expected R\'enyi-2 of radiation obeys a data-processing inequality and is nondecreasing; see App.~D. A Page-like turnover therefore implies a breakdown of these hypotheses (late-time microstate dependence and CP-indivisible backflow). We quantify non-Markovianity via a BLP-type witness $\mathcal{N}=\int_{\dot D>0}\!\dot D\,\mathrm{d}t$ using distinguishability $D(t)$ for optimized pairs.

\noindent\textbf{Theorem 1 (R\'enyi-2 monotonicity).} If the reduced dynamics of the radiation is CP-divisible and unital in the relevant subspace, then $\mathbb{E}[S^{(2)}_{\rm rad}(t+\Delta t)]\ge \mathbb{E}[S^{(2)}_{\rm rad}(t)]$ for all coarse-grained bins $\Delta t$.
\emph{Sketch.} CP-divisibility implies a CPTP map between bins; unitality ensures data-processing of collision entropy. Taking expectation over the scrambling ensemble yields the stated monotonicity. Full proofs are given in App.~D.
